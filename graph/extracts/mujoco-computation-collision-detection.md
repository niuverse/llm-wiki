...
[View this page](../_sources/computation/index.rst.txt "View this page")

Toggle Light / Dark / Auto color theme

Toggle table of contents sidebar

# Computation[#](#computation "Link to this heading")
...
## Soft contact model[#](#soft-contact-model "Link to this heading")
...
### Physical realism and soft contacts[#](#physical-realism-and-soft-contacts "Link to this heading")

Many of the advantages of our contact model can be traced to the fact that we drop the strict complementarity constraint
at the heart of the LCP formulation. We will call this family of models convex; see [References](#references) for related work.
...
### Computational efficiency[#](#computational-efficiency "Link to this heading")
...
### Continuous time[#](#continuous-time "Link to this heading")
...
### Inverse dynamics and optimization[#](#inverse-dynamics-and-optimization "Link to this heading")
...
## General framework[#](#general-framework "Link to this heading")

Our notation is summarized in the table below. Additional notation specific to constraints will be introduced later.
When available, we also show the fields of main data structures [mjModel](../APIreference/APItypes.html#mjmodel) and [mjData](../APIreference/APItypes.html#mjdata) corresponding to the
...
earlier arm model [example](../overview.html#examples) the model has \(\nv = 13\) degrees of freedom: 3 for the ball joint, one
for each of the 4 hinge joints, and 6 for the free-floating object. They appear in the same order in all system-level
vectors and matrices whose dimensionality is \(\nv\). The data corresponding to a given model element can be
recovered via indexing operations as illustrated in the [Clarifications](../overview.html#clarifications) section in the Overview chapter. Vectors
and matrices with dimensionality \(\nc\) are somewhat different because the active [constraints](#constraint)
...
in most models). In that case \(\dot{q}\) does not equal \(v\), at least not in the usual sense. Instead one has
to consider the group of rigid body orientations \(SO(3)\) - which has the geometry of a unit sphere in 4D space.
Velocities live in the 3D tangent space to this sphere. This is taken into account by all internal computations. For
custom computations, MuJoCo provides the function [mj\_differentiatePos](../APIreference/APIfunctions.html#mj-differentiatepos) which “subtracts” two position vectors with
...
(1)[#](#equation-eq-motion "Permalink to this equation")\[M \dot{v} + c = \tau + J^T f
\]

The Jacobian establishes the relationship between quantities in joint and constraint coordinates. It maps motion vectors
(velocities and accelerations) from joint to constraint coordinates: the joint velocity \(v\) maps to velocity
\(J v\) in constraint coordinates. The transpose of the Jacobian maps force vectors from constraint to joint
coordinates: the constraint force \(f\) maps to force \(J^T f\) in joint coordinates.

The joint-space inertia matrix \(M\) is always invertible. Therefore once the constraint force \(f\) is known,
...
* The applied force \(\tau\) includes [passive](#gepassive) forces from spring-dampers and fluid dynamics,
  [actuation](#geactuation) forces, and additional forces specified by the user.
* The bias force \(c\) includes Coriolis, centrifugal and gravitational forces. Their sum is computed using the
  Recursive Newton-Euler (RNE) algorithm with acceleration set to 0.
* The joint-space inertia matrix \(M\) is computed using the Composite Rigid-Body (CRB) algorithm. This matrix is
  usually quite sparse, and we represent it as such, in a custom format tailored to kinematic trees.
* Since we often need to multiply vectors by the inverse of \(M\), we compute its \(L^T D L\) factorization in
  a way that preserves sparsity. When a quantity of the form \(M^{-1} x\) is needed later, it is computed via
...
detailed summary of the [simulation pipeline](#pipeline) is given at the end of the chapter.

### Actuation model[#](#actuation-model "Link to this heading")

MuJoCo provides a flexible actuator model. All actuators are single-input-single-output (SISO). The input to actuator
\(i\) is a scalar control \(u\_i\) specified by the user. The output is a scalar force \(p\_i\) which is
mapped to joint coordinates by a vector of moment arms determined by the transmission. An actuator can also have
activation state \(w\_i\) with its own dynamics. The control inputs for all actuators are stored in `mjData.ctrl`,
...
actuator works. The user can set them independently for maximum flexibility, or use [Actuator shortcuts](../modeling.html#cactshortcuts) which instantiate common actuator types.

#### Transmission[#](#transmission "Link to this heading")

Each actuator has a scalar length \(l\_i(q)\) defined by the type of transmission and its parameters. The gradient
...
is attached; the possible attachment object types are joint, tendon, jointinparent,
slider-crank, site, and body.

joint and tendon
:   The joint and tendon transmission types act as expected and correspond to the actuator applying forces or
    torques to the target object. Ball joints are special, see the [actuator/general/joint](../XMLreference.html#actuator-general-joint)
    documentation for more details.

jointinparent
:   The jointinparent transmission is unique to ball and free joint and asserts that rotation should be measured
    in the parent rather than child frame.

slider-crank
:   slider-crank [transmissions](https://en.wikipedia.org/wiki/Slider-crank_linkage) transform a linear force to
    a torque, as in a piston-driven combustion engine. [This model](https://github.com/google-deepmind/mujoco/blob/main/model/slider_crank/slider_crank.xml) contains pedagogical
...
body
:   body transmission corresponds to applying forces at contact points belonging to a body, in
    order to model vacuum grippers and biomechanical adhesive appendages. For more information about adhesion, see the
    [adhesion](../XMLreference.html#actuator-adhesion) actuator documentation. These transmission targets have a fixed zero length
    \(l\_i(q) = 0\).

site
:   Site transmissions correspond to applying a Cartsian force/torque in the frame of a site. When a refsite is not
    defined (see below), these targets have a fixed zero length \(l\_i(q) = 0\) and are useful for modeling jets and
    propellors: forces and torques which are fixed to the site frame.

    If a site transmission is defined with the optional refsite attribute, forces and torques are applied in
    ...
    reference frame. This length can then be controlled with a position actuator, allowing for Cartesian
    end-effector control. See the [refsite](../XMLreference.html#actuator-general-refsite) documentation for more details.

#### Stateful actuators[#](#stateful-actuators "Link to this heading")

Some actuators such as pneumatic and hydraulic cylinders as well as biological muscles have an internal state called
“activation”. This is a true dynamic state, beyond the joint positions \(q\) and velocities \(v\). Including
such actuators in the model results in 3rd-order dynamics. We denote the vector of actuator activations \(w\).
They have some first-order dynamics

\[\dot{w}\_i \left( u\_i, w\_i, l\_i, \dot{l}\_i \right)
\]
...
\[\begin{aligned}
\text{integrator}: & & \dot{w}\_i &= u\_i \\
\text{filter}: & & \dot{w}\_i &= (u\_i - w\_i) / \texttt{t} \\
\text{filterexact}: & & \dot{w}\_i &= (u\_i - w\_i) / \texttt{t} \\
\text{muscle}: & & \dot{w}\_i &= \textrm{muscle}(u\_i, w\_i, l\_i, \dot{l}\_i)
\end{aligned}
\]

where \(\texttt{t}\) is an actuator-specific time-constant stored in `mjModel.actuator_dynprm`. In addition, the
type can be “user”, in which case \(w\_i\) is computed by the user-defined callback [mjcb\_act\_dyn](../APIreference/APIglobals.html#mjcb-act-dyn). The type can
also be “none” which corresponds to a regular actuator with no activation state. The dimensionality of \(w\) equals
the number of actuators whose activation type is different from “none”.

For more information regarding muscle activation dynamics, see [Muscles](../modeling.html#cmuscle).

For `filterexact` activation dynamics, Euler integration of \(\dot{w}\) is replaced with the analytic integral:

\[\begin{aligned}
\text{filter}: & & w\_{i+1} &= w\_i + h (u\_i - w\_i) / \texttt{t} \\
\text{filterexact}: & & w\_{i+1} &= w\_i + (u\_i - w\_i) (1 - e^{-h / \texttt{t}}) \\
\end{aligned}
\]

The two expressions converge to the same value in the \(h \rightarrow 0\) limit. Note that Euler-integrated filters
diverge for \(\texttt{t} < h\), while exactly-integrated filters are stable for any positive \(\texttt{t}\).

[actearly](../XMLreference.html#actuator-general-actearly):
:   If the [actearly](../XMLreference.html#actuator-general-actearly) attribute is set to “true”, `mjData.actuator_force` is computed
    based on \(w\_{i+1}\) (the next activation), reducing the delay between changes to \(u\) and their effects on
    the acceleration by one time step (so the total dynamics are second-order rather than third order).

#### Force generation[#](#force-generation "Link to this heading")

Each actuator generates a scalar force \(p\_i\) which is some function

\[p\_i \left( u\_i, w\_i, l\_i, \dot{l}\_i \right)
\]
...
\[p\_i = (a w\_i \; \text{or} \; a u\_i) + b\_0 + b\_1 l\_i + b\_2 \dot{l}\_i
\]

Here \(a\) is an actuator-specific gain parameter and \(b\_0, b\_1, b\_2\) are actuator-specific bias
...
and bias terms by installing the callbacks [mjcb\_act\_gain](../APIreference/APIglobals.html#mjcb-act-gain) and [mjcb\_act\_bias](../APIreference/APIglobals.html#mjcb-act-bias) and setting the gain and
...
\[\sum\_i \nabla l\_i(q) \; p\_i \left(u\_i, w\_i, l\_i(q), \dot{l}\_i(q, v) \right)
\]
...
### Passive forces[#](#passive-forces "Link to this heading")
...
callback [mjcb\_passive](../APIreference/APIglobals.html#mjcb-passive) and add forces to `mjData.qfrc_passive` that may increase energy. This will not interfere
...
* Spring-dampers in joints and tendons. See the following attributes for details.
   ...
   **Joints:**
  [stiffness](../XMLreference.html#body-joint-stiffness), [springref](../XMLreference.html#body-joint-springref),
  [damping](../XMLreference.html#body-joint-damping), [springdamper](../XMLreference.html#body-joint-springdamper).

   **Tendons:** [stiffness](../XMLreference.html#tendon-spatial-stiffness),
  [springlength](../XMLreference.html#tendon-spatial-springlength), [damping](../XMLreference.html#tendon-spatial-damping).
* Gravity compensation forces. See the body [gravcomp](../XMLreference.html#body-gravcomp) attribute for details.
* Fluid forces exerted by the surrounding medium. See the [Fluid forces](fluid.html) chapter for details.

#### Polynomial forces[#](#polynomial-forces "Link to this heading")

Nonlinear stiffness ([joints](../XMLreference.html#body-joint-stiffness), [tendons](../XMLreference.html#tendon-spatial-stiffness)) and
damping ([joints](../XMLreference.html#body-joint-damping), [tendons](../XMLreference.html#tendon-spatial-damping)) are defined by polynomials
\(f\) of order [mjNPOLY + 1](../APIreference/APIglobals.html#glnumericsizes). The actual force applied to the system is \(-f\), meaning
...
**Anti-symmetrization**
:   The damping polynomial uses anti-symmetrized even-powered monomials (e.g., \(v^2 \rightarrow v|v|\)) so that the
    function is odd: \(f(-v) = -f(v)\). This guarantees the force reverses direction with velocity. This formulation
...
:   In both cases, sensible choices of coefficients often satisfy the **sign-preservation** condition
    \(z \cdot f(z) \geq 0\). This condition is equivalent to requiring that the integral of \(f\) (Potential
...
**mjModel fields**
:   Although MJCF accepts the coefficients as a single array (as does the [mjs layer](../APIreference/APItypes.html#mjsjoint)),
    the linear coefficient in `mjModel` is stored separately from the higher-order ones.
    For example, if [joint/stiffness](../XMLreference.html#body-joint-stiffness) = “a b c”,
...
acceleration \(a=\dot{v}\) as well as the actuator activations \(\dot{w}\) when present in the model. These are
used to advance the simulation time from \(t\) to \(t+h\), and to update the state variables \(q, v, w\).
...
(2)[#](#equation-eq-explicit "Permalink to this equation")\[\begin{aligned}
\textrm{activation:}\quad w\_{t+h} &= w\_t + h \dot{w}\_t \\
\textrm{velocity:}\quad v\_{t+h} &= v\_t + h a\_t \\
\textrm{position:}\quad q\_{t+h} &= q\_t + h v\_t
\end{aligned}\]

Note that in the presence of quaternions, the operation \(q\_t + h v\_t\) is more involved than a simple summation, as
the dimensionalities of \(q\) and \(v\) are different. The reason explicit Euler is not implemented is that the
following formulation, known as *semi-implicit* Euler is [strictly better](https://en.wikipedia.org/wiki/Semi-implicit_Euler_method), and standard in physics simulation:

(3)[#](#equation-eq-semimplicit "Permalink to this equation")\[ \begin{aligned}
v\_{t+h} &= v\_t + h a\_t \\
q\_{t+h} &= q\_t + h v\_{\color{red}t+h}
\end{aligned}\]

Comparing [(2)](#equation-eq-explicit) and [(3)](#equation-eq-semimplicit), we see that in semi-implicit Euler, the position is updated using
the *new* velocity. *Implicit* Euler means:

(4)[#](#equation-eq-implicit "Permalink to this equation")\[\begin{aligned}
v\_{t+h} &= v\_t + h a\_{\color{red}t+h} \\
q\_{t+h} &= q\_t + h v\_{t+h}
\end{aligned}\]

Comparing [(3)](#equation-eq-semimplicit) and [(4)](#equation-eq-implicit), we see that the acceleration \(a\_{t+h}=\dot{v}\_{t+h}\) on the
...
acceleration as a function of velocity, \(a\_t = a(v\_t)\), the velocity update we aim to approximate is

\[v\_{t+h} = v\_t + h a(v\_{t+h})
\]

This is a non-linear equation in the unknown vector \(v\_{t+h}\) and can be solved numerically at each time step
using a first-order expansion of \(a(v\_{t+h})\) around \(v\_t\). Recall that the forward dynamics are

(5)[#](#equation-eq-forward "Permalink to this equation")\[a(v) = M^{-1} \big(\tau(v) - c(v) + J^T f(v)\big)\]
...
\[\begin{aligned}
v\_{t+h} &= v\_t + h a(v\_{t+h}) \\
&\approx v\_t + h \big( a(v\_t) + {\partial a(v) \over \partial v} \cdot (v\_{t+h}-v\_t) \big) \\
&= v\_t + h a(v\_t) + h M^{-1} D \cdot (v\_{t+h}-v\_t)
\end{aligned}
\]

Premultiplying by \(M\) and rearranging yields

\[(M-h D) v\_{t+h} = (M-h D) v\_t + h M a(v\_t)
\]

Solving for \(v\_{t+h}\), we obtain the implicit-in-velocity update

(6)[#](#equation-eq-implicit-update "Permalink to this equation")\[\begin{aligned}
v\_{t+h} &= v\_t + h \widehat{M}^{-1} M a(v\_t) \\
\widehat{M} &\equiv M-h D
\end{aligned}\]

Midpoint integration for free bodies in vacuum
:   The implicit-in-velocity update [(6)](#equation-eq-implicit-update) treats the acceleration as a function of velocity and
    ...
    \(\omega \in \mathbb{R}^3\) and diagonal inertia tensor \(I = \text{diag}(I\_1, I\_2, I\_3)\). The rotational
    dynamics are given by [Euler’s rotation equation](https://en.wikipedia.org/wiki/Euler%27s_equations_%28rigid_body_dynamics%29):

    \[I \dot{\omega} + \omega \times I\omega = \tau
    \]

    where \(\tau\) is the external torque in the principal-axis frame.
    Evaluating the velocities at the midpoint, \(\omega\_\text{mid} = (\omega\_t + \omega\_{t+h})/2\), gives:

    \[\frac{2}{h} I (\omega\_\text{mid} - \omega\_t) + \omega\_\text{mid} \times I \omega\_\text{mid} = \tau
    \]

    This is a system of 3 nonlinear equations in 3 unknowns \(\omega\_\text{mid}\), solved at each timestep using
    Newton’s method with a backtracking line search. After solving, the new velocity is recovered as
    \(\omega\_{t+h} = 2\omega\_\text{mid} - \omega\_t\).

    **Properties.** The midpoint method preserves all [quadratic first integrals](https://doi.org/10.1007/3-540-30666-8) of the ODE. For Euler’s equations, these are the
    kinetic energy \(H = \frac{1}{2}\omega^T I\omega\) and the squared angular momentum
    \(C = \frac{1}{2}|I\omega|^2\), both conserved exactly in the absence of external torque. Since \(C\) is the
    Casimir function of the [Lie-Poisson](https://en.wikipedia.org/wiki/Poisson_bracket) structure, the midpoint
    ...
    **Eligibility.** Midpoint integration is only applied when using the `implicitfast` integrator, to
    free bodies with no child bodies, and only when the medium has zero [density](../XMLreference.html#option-density) and
    [viscosity](../XMLreference.html#option-viscosity).
    ...
    **Disabling.** Because midpoint integration solves a nonlinear equation for the next velocity, it breaks the linear
    ...
    setting the [invdiscrete](../XMLreference.html#option-flag-invdiscrete) flag disables midpoint integration, and also provides a
    general opt-out mechanism for this integrator.

#### Integrators[#](#integrators "Link to this heading")

MuJoCo supports four integrators: three single-step integrators and the multi-step 4th order Runge-Kutta integrator.
All three single-step integrators in MuJoCo use the update [(6)](#equation-eq-implicit-update), with different definitions of the
\(D\) matrix, which is always computed analytically.

Semi-implicit with implicit joint damping (`Euler`)
:   For this method, \(D\) only includes derivatives of joint damping. Note that in this case \(D\) is diagonal
    and \(\widehat{M}\) is symmetric, so \(L^TL\) decomposition (a variant of Cholesky) can be used. This
    factorization is stored in `mjData.qH`. If the model has no joint damping or the
    [eulerdamp](../XMLreference.html#option-flag-eulerdamp) disable flag is set, implicit damping is disabled and the semi-implicit
    update [(3)](#equation-eq-semimplicit) is used, rather than [(6)](#equation-eq-implicit-update), avoiding the additional factorization of
    \(\widehat{M}\) (*additional* because \(M\) is already factorized for the acceleration update
    [(5)](#equation-eq-forward)).

Implicit-in-velocity (`implicit`)
:   For this method, \(D\) includes derivatives of all forces except the constraint forces \(J^T f(v)\). These
    ...
    future version. Additionally, we restrict \(D\) to have the same sparsity pattern as \(M\), for computational
    efficiency. This restriction will exclude damping in tendons which connect bodies that are on different branches of
    the kinematic tree. Since \(D\) is not symmetric, we cannot use Cholesky factorization, but because \(D\) and
    \(M\) have the same sparsity pattern corresponding to the topology of the kinematic tree, reverse-order
    \(LU\) factorization of \(\widehat{M}\) is guaranteed to have [no fill-in](https://link.springer.com/book/10.1007/978-1-4899-7560-7). This factorization is stored `mjData.qLU`.

Fast implicit-in-velocity (`implicitfast`)
:   For this method, \(D\) includes derivatives of all forces used in the implicit method, with the exception of the
    centripetal and Coriolis forces \(c (v)\) computed by the RNE algorithm. Additionally, it is symmetrized \(D
    ...
    derivatives are also the main source of asymmetry of \(D\), by dropping them and symmetrizing, we can use the
    faster \(L^TL\) rather than \(LU\) decomposition.
    The `implicitfast` integrator applies [midpoint integration](#gemidpoint) to eligible free bodies in vacuum,
...
:   One advantage of our continuous-time formulation is that we can use higher order integrators such as Runge-Kutta or
    multistep methods. MuJoCo implements the fixed-step [4th-order Runge-Kutta method](https://en.wikipedia.org/wiki/Runge%E2%80%93Kutta_methods#Derivation_of_the_Runge–Kutta_fourth-order_method), though
    users can easily implement other integrators by calling [mj\_forward](../APIreference/APIfunctions.html#mj-forward) and integrating accelerations themselves.
    We have observed that for energy-conserving systems ([example](../_static/pendulum.xml)), RK4 is qualitatively
...
[timestep](../XMLreference.html#option-timestep)
:   The accuracy and stability of all integrators can be improved by reducing the time step \(h\).
...
[integrator](../XMLreference.html#option-integrator)
:   Summary: The recommended integrator is `implicitfast` which usually has the best tradeoff of stability and
    ...
    **implicit**:
    :   The benefit over `implicitfast` is the implicit integration of Coriolis and centripetal forces for *coupled*
        rotational systems such as multi-link pendula. Note that `implicit` does not apply [midpoint
        integration](#gemidpoint) (only `implicitfast` does), but its RNE derivatives provide comparable stability
        for free-body rotation. For example, [gyroscopic.xml](../_static/gyroscopic.xml) shows an ellipsoid rolling
        on an inclined plane; both `implicitfast` and `implicit` handle this case well, while `Euler` quickly
        diverges.

    **RK4**:
    :   This integrator is best for systems which are energy conserving, or almost energy-conserving. [pendulum.xml](../_static/pendulum.xml) shows a complicated pendulum mechanism which diverges quickly using `Euler` or
...
### The State[#](#the-state "Link to this heading")

To complete our description of the general framework we will quickly discuss the notion of *state*. MuJoCo has a
compact, well-defined internal state which, together with the [deterministic pipeline](#pireproducibility), means
that operations like (re)setting the state and computing dynamics derivatives are also well-defined.

The state is entirely encapsulated in the [mjData](../APIreference/APItypes.html#mjdata) struct and consists of several components. The components are
enumerated in [mjtState](../APIreference/APItypes.html#mjtstate) as bit flags. Concatenated state vectors can be conveniently read from and written into
[mjData](../APIreference/APItypes.html#mjdata) using [mj\_getState](../APIreference/APIfunctions.html#mj-getstate) and [mj\_setState](../APIreference/APIfunctions.html#mj-setstate), respectively.

More detail can be found in the [State and Control](../programming/simulation.html#sistatecontrol) section in the Simulation chapter.

## Constraint model[#](#constraint-model "Link to this heading")

MuJoCo has a very flexible constraint model, which is nevertheless handled in a uniform way by the
[solver](#solver) described later. Here we explain what the individual constraints are conceptually, and how they
are laid out in the system-level vector and matrices with dimensionality \(\nc\). Each conceptual constraint can
contribute one or more scalar constraints towards the total count \(\nc\), and each scalar constraint has a
...
### Equality[#](#equality "Link to this heading")

MuJoCo can model equality constraints in the general form \(r(q) = 0\) where \(r\) can be any differentiable
scalar or vector function of the position vector \(q\). It has the semantics of a residual. The solver can actually
work with non-holonomic constraints as well, but we do not yet have such constraint types defined. Each equality
constraint contributes \(\dim(r)\) elements to the total constraint count \(\nc\). The corresponding block in
\(J\) is simply the Jacobian of the residual, namely \(\partial r / \partial q\). Note that due to the
properties of quaternions, differentiation with respect to \(q\) produces vectors of size \(\nv\) rather than
\(\nq\).
...
:   This constraint welds two bodies together, suppressing all relative degrees of freedom between them. The relative
    ...
    followed by a 3D orientation component. The latter is defined as \(\sin(\theta/2)
    (x, y, z)\) where \(\theta\) is the rotation angle in radians and \((x, y, z)\) is the unit vector
    ...
    result if we used \(\theta\) instead of \(\sin(\theta/2)\). It does have a drawback though: when the angle
...
:   This constraint applies only to scalar joints: hinge and slide. It can be used to lock one joint into a constant
    ...
    defined as follows. Suppose \(y\) is the position of the first joint and \(x\) is the position of the second
    ...
    \[y-y\_0 = a\_0 + a\_1 \left( x-x\_0 \right) + a\_2 \left( x-x\_0 \right)^2 +
    a\_3 \left( x-x\_0 \right)^3 + a\_4 \left( x-x\_0 \right)^4
    \]

    where \(a\_0, \ldots, a\_4\) are coefficients defined in the model. If the constraint involves only one joint, it
    reduces to \(y-y\_0 = a\_0\).
...
### Friction loss[#](#friction-loss "Link to this heading")
...
with it; so we formally set the corresponding components of \(r(q)\) to zero. Indeed we will see later that our
...
Jacobian of the joint position (or tendon length) with respect to \(q\). For scalar joints this is a vector of 0s
...
\(r(q)\) is the distance between the current position/length and the closer of the two limiting values specified in
...
constraint force depends on distance through the solver [parameters](#soparameters) described later.
...
:   Limits can be defined for scalar joints (hinge and slide) as well as for ball joints. Scalar joints are treated as
    ...
    quaternion, i.e., the vector \((\theta x, \theta y, \theta z)\) where \(\theta\) is the rotation angle and
    \((x, y, z)\) is the unit vector corresponding to the rotation axis. The limit is applied to the absolute value
    of the rotation angle \(\theta\). At runtime the limit is determined by the larger of the two range parameters.
...
that point, both expressed in global coordinates. The first (\(x\)) axis of this frame is the contact normal
direction, while the remaining (\(y\) and \(z\)) axes define the tangent plane. One might have expected the
normal to correspond to the \(z\) axis, as in MuJoCo’s visualization convention, but we support frictionless
...
| --- | --- |
| `condim` | Dimensionality of the contact force/torque in the contact frame.   It can be 1, 3, 4 or 6. |
...
| `gap` | An additional detection buffer beyond `margin`. Contacts with distance between `margin` and `margin + gap` are included in `mjData.contact` as inactive contacts, but no contact forces are generated. This is useful for action-at-a-distance effects, for example by [adhesion](../XMLreference.html#actuator-adhesion) actuators. |
| `solref` and `solimp` | [Solver](#solver) parameters, explained later. |

#### margin and gap[#](#margin-and-gap "Link to this heading")

Each geom has a `margin` and a `gap` parameter, defined in the table above. The values for both parameters are
[summed](../modeling.html#ccontact) when considering contact between the two geoms. Together they define three regimes of contact
detection and force generation, illustrated in the figure below.

[![../_images/margin_gap_light.svg](../_images/margin_gap_light.svg)](../_images/margin_gap_light.svg)
[![../_images/margin_gap_dark.svg](../_images/margin_gap_dark.svg)](../_images/margin_gap_dark.svg)
...
* **Inactive contact** (`margin` < distance ≤ `margin + gap`): A contact is detected and included in
  ...
  computations, for example by [adhesion](../XMLreference.html#actuator-adhesion) actuators.
...
#### condim[#](#condim "Link to this heading")
...
#### Friction cones[#](#friction-cones "Link to this heading")

Now we describe the friction cones and the corresponding Jacobians more formally. In this section only, let \(f\)
denote the vector of constraint forces for a single contact (as opposed to the system-level vector of constraint
forces), \(\mu\) the vector of friction coefficients, and \(n\) the contact dimensionality condim. For
\(n > 1\) the elliptic and pyramidal friction cones are defined as

\[\begin{aligned}
\text{elliptic cone}: & & \mathcal{K} &=
\left\{ f \in \mathbb{R}^n : f\_1 \geq 0,
f\_1^2 \geq \sum\_{i=2}^n {f\_i^2 / \mu\_{i-1}^2} \right\} \\
\text{pyramidal cone}: & & \mathcal{K} &=
\left\{ f \in \mathbb{R}^{2(n-1)} : f \geq 0 \right\} \\
\end{aligned}
\]

The vector inequality in the pyramidal cone definition is meant element-wise. For \(n=1\) both cones are defined as
the non-negative ray (which is a special case of a cone). Note that the system-level friction cone discussed in the
solver section below will also be denoted \(\mathcal{K}\). It is the product of the friction cones for the
...
each component of \(f\). The basis vectors are spatial vectors: 3D force followed by 3D torque. Arranging the basis
vectors into the columns of a matrix \(E\), the force/torque generated by the constraint force in the contact frame
is \(E f\). The matrix of basis vectors is constructed as follows.

[![../_images/contact_frame.svg](../_images/contact_frame.svg)](../_images/contact_frame.svg)
[![../_images/contact_frame_dark.svg](../_images/contact_frame_dark.svg)](../_images/contact_frame_dark.svg)

The figure illustrates the full basis set corresponding to the case \(n = 6\). Otherwise we use only the first
\(n\) or \(2(n-1)\) columns depending on the cone type. Elliptic cones are easier to understand. Since the
matrix \(E\) is the identity matrix, the first three components of \(f\) are forces acting along the axes of the
...
\(S\) which maps joint velocities \(v\) to spatial velocities \(S v\) expressed in the contact frame. This
is done by considering the contact point as belonging to one or the other geom, computing its spatial Jacobian, and
subtracting these two Jacobians to obtain \(S\). The convention we use is that the contact force acts from the first
towards the second geom, so the spatial Jacobian for the first geom has negative sign. The contact Jacobian is then
\(E^T S\). As with all other constraints, this matrix is inserted into the system-level Jacobian \(J\).

## Constraint solver[#](#constraint-solver "Link to this heading")
...
accelerations \(\dot{v}\) where the constraint forces are implicit. We then transform the primal problem over
...
Gauss principle states that if we have unconstrained dynamics \(M \dot{v} = \tau\) and impose acceleration
constraint \(J \dot{v} = \ar\), the resulting acceleration will be

\[\dot{v} = \arg \min\_x \left\| x-M^{-1} \tau \right\|^2\_M \\
\textrm{subject to} \; J x = \ar
\]

where the weighted \(L\_2\) norm is the usual \(\|x\|^2\_M = x^T M x\). Thus the constraint causes the smallest
possible deviation from the unconstrained acceleration \(M^{-1}\tau\), where the metric for measuring deviations in
...
The index sets will be used to refer to parts of vectors and matrices. For example, \(J\_\mathcal{C}\) is the
sub-matrix of all rows of the Jacobian that correspond to contact constraints.

### Primal problem[#](#primal-problem "Link to this heading")

We first formulate the optimization problem whose solution yields the constrained acceleration \(\dot{v}\), and then
explain what it means and why it makes sense. That problem is

(7)[#](#equation-eq-primal "Permalink to this equation")\[(\dot{v}, \dot{\omega}) = \arg \min\_{(x, y)}
\left\|x-M^{-1}(\tau-c)\right\|^2\_M +
\left\|y-\ar\right\|^{\text{Huber}(\eta)}\_{R^{-1}} \\
\textrm{subject to} \;
J\_\mathcal{E} x\_\mathcal{E} - y\_\mathcal{E} = 0, \;
J\_\mathcal{F} x\_\mathcal{F} - y\_\mathcal{F} = 0, \;
J\_\mathcal{C} x\_\mathcal{C} - y\_\mathcal{C} \in \mathcal{K}^\*
\]

The new players here are the diagonal regularizer \(R > 0\) which makes the constraints soft, and the reference
acceleration \(\ar\) which stabilizes the constraints; the latter is a spring-damper defined in the
[Parameters](#soparameters) section below. It is similar in spirit to Baumgarte stabilization,
but instead of adding a constraint force directly, it modifies the optimization problem whose solution is the constraint
force. Since this problem is itself constrained, the relation between \(\ar\) and \(f\) is generally non-linear.
The quantities \(R\) and \(\ar\) are computed from the solver [parameters](#soparameters) as described
later. For now we assume they are given.

The optimization variable \(x\) stands for acceleration as in the Gauss principle, while \(y\) is a slack
variable in constraint space. It is needed to model soft constraints. If we forced the solution to reach
\(y = \ar\), which we could do by taking the limit \(R \to 0\), we would obtain a hard constraint model. This
limit is not allowed in MuJoCo, but nevertheless one can construct models that are phenomenologically hard.

The symbol \(\mathcal{K}^\*\) denotes the dual to the friction cone. It is motivated by mathematical reverse
engineering: we want to recover the constraint \(f \in \mathcal{K}\) after taking the dual of our primal problem,
...
friction loss parameters. Setting \(\eta = \infty\) recovers the quadratic norm; we use this convention for all
...
We now relate problem [(7)](#equation-eq-primal) more closely to the Gauss principle, and give physical meaning to the slack
variables. Consider an augmented dynamical system with position \((q, z)\) and velocity \((v, \omega)\). The new
state variables correspond to deformation dynamics. Similarly to the original system where \(v\) is not the same as
\(\dot{q}\), here \(\omega\) is not the same as \(\dot{z}\), although the reason is different. Deformation
...
deformation variables we have \(\dot{z} = \omega\). For friction loss and frictional components of elliptic cones
however, we have \(z = 0\) while \(\omega \neq 0\). This is because even though there may be motion in the
...
general equations of motion [(1)](#equation-eq-motion) now become
...
Thus \(R\) has the meaning of inverse deformation inertia, while \(\ar\) has the meaning of unforced deformation
acceleration.
...
the functional dependence of the regularizer and the reference acceleration as \(R(q)\) and \(\ar(q, v)\). This
makes problem [(7)](#equation-eq-primal) dependent only on \((q, v, \tau)\), and so the original dynamics are not actually
affected by the deformation dynamics. Since the general constraint model we developed up to now makes no assumptions
about how \(R\) and \(\ar\) are computed, our choice is consistent and improves simulator efficiency.
Nevertheless, given that these quantities turned out to be related to the deformation dynamics, it may be more natural
to define them as \(R(z)\) and \(\ar (z, \omega)\) and simulate the entire augmented system. Below we clarify
...
constraint force \(f\) equals the quantity \(f^+\) defined in the parameter section below. Then the deformation
states become static functions of the joint position and velocity, namely \(z = r(q)\) and \(\omega = J(q) v\).
...
### Reduced primal problem[#](#reduced-primal-problem "Link to this heading")

The primal problem defined in [(7)](#equation-eq-primal), as well as the dual problem we will obtain later, are both constrained
...
optimization over accelerations. If \(x\) in [(7)](#equation-eq-primal) is given, minimizing over \(y\) can be done in
closed form. This also eliminates the constraints, in the sense that the solution for \(y\) automatically satisfies
the constraints. We are then left with an unconstrained optimization problem over \(x\) which can be solved with
more efficient algorithms.

The reduction is based on the fact that minimization over \(y\) in [(7)](#equation-eq-primal) comes down to finding the nearest
...
(8)[#](#equation-eq-reduced "Permalink to this equation")\[\dot{v} = \arg \min\_{x} \left\|x-M^{-1}(\tau-c)\right\|^2\_M +
s \left( J x - \ar \right)
\]

The function \(s(\cdot)\) plays the role of a soft-constraint penalty. It can be shown to be convex and
...
[(1)](#equation-eq-motion), we see that the constraint forces \(f\) are given by the negative gradient of the function
\(s(\cdot)\). Differentiating one more time with respect to \(\dot{v}\) yields
...
\(s(\cdot)\) and its derivatives are key to the MuJoCo physics model.

### Dual problem[#](#dual-problem "Link to this heading")
...
(9)[#](#equation-eq-dual "Permalink to this equation")\[f = \arg\min\_\lambda \frac{1}{2} \lambda^{T} \left( A+R \right) \lambda +
...
The constraint set \(\Omega\) is as follows. \(\lambda\_\mathcal{E}\) is unconstrained, because it is the
Lagrange multiplier for an equality constraint in the primal problem. For friction loss we have the box constraint
\(\left|\lambda\_\mathcal{F}\right| \leq \eta\) applied element-wise. For contacts we have
\(\lambda\_\mathcal{C} \in \mathcal{K}\). For pyramidal cones this is simply \(\lambda\_\mathcal{C} \geq 0\),
while for elliptic cones it is a second-order cone constraint. While \(A\) is only symmetric positive semi-definite,
\(R\) is symmetric positive definite by construction, and so the above quadratic cost is strictly convex. Thus for
pyramidal friction cones we have a convex box-constrained quadratic program, and for elliptic friction cones we have a
mixture of box constraints and second-order cone constraints. The [algorithms](#soalgorithms) for solving this
...
dynamics we have access to \((q, v, \dot{v})\) instead of \((q, v, \tau)\), so the unconstrained acceleration
\(\au\) is unknown. However we can compute the constrained acceleration
...
\[f = \arg \min\_\lambda \frac{1}{2} \lambda^{T} R \lambda +
...
(10)[#](#equation-eq-identity "Permalink to this equation")\[\ac = \au + Af
\]

This key identity is essentially Newton’s second law projected in constraint space. It is derived by moving the term
\(c\) in the equations of motion [(1)](#equation-eq-motion) to the right hand side, multiplying by \(J M^{-1}\) from the
left, adding \(\dot{J} v\) to both sides, and substituting the above definitions of \(A, \au, \ac\). Computing
\(\dot{J} v\) requires differentiating the constraint Jacobian with respect to time, which is nontrivial.
Although this term cancels in the identity [(10)](#equation-eq-identity) and so does not affect the forward-inverse comparison, its
...
Note that the quadratic term in the inverse problem is weighted by \(R\) instead of \(A+R\). This is the key
structural insight: the \(A\) matrix cancels entirely, leaving only \(R\) in the quadratic term. Two
consequences follow. First, in the limit \(R \to 0\) corresponding to hard constraints the inverse is no longer
defined, as one would expect. Second, the inverse problem is diagonal, i.e., it decouples into independent optimization
problems over the individual constraint forces. Since \(R\) is diagonal, no matrix inversion or factorization is
needed – the inverse dynamics require no optimization at all, only analytical formulas. The only remaining coupling is
due to the constraint set \(\Omega\), but that set is also decoupled over the conceptual constraints discussed
...
[paper](https://scholar.google.com/scholar?cluster=9217655838195954277). It requires a certain coupling of the
diagonal values of \(R\), which is automatically enforced by MuJoCo so as to enable an exact analytical inverse for
every model.
...
and CG solvers work with the reduced primal formulation [(8)](#equation-eq-reduced), while the PGS solver works with the dual
formulation [(9)](#equation-eq-dual). Note that numerical solvers are only needed in forward dynamics. Inverse dynamics are handled
analytically.
...
:   This algorithm uses the non-linear conjugate gradient method with the Polak-Ribiere-Plus formula (non-negative
    \(\beta\)). Line-search is exact, using Newton’s method in one dimension with analytical second derivatives on
...
:   This is the most common algorithm used in physics simulators, and used to be the default in MuJoCo, until we
    ...
    [![../_images/gPGS.svg](../_images/gPGS.svg)](../_images/gPGS.svg)
    [![../_images/gPGS_dark.svg](../_images/gPGS_dark.svg)](../_images/gPGS_dark.svg)
...
:   This is not a standalone solver but a post-processing step, enabled by setting `noslip_iterations` to a positive
    value in [option](../XMLreference.html#option). After the main solver (Newton, CG, or PGS) has converged, the NoSlip solver
    re-solves the friction dimensions only, using a PGS sweep with \(R = 0\) (i.e., hard constraints) in those
...
### Constraint islands[#](#constraint-islands "Link to this heading")

[![../_images/island.svg](../_images/island.svg)](../_images/island.svg)

Consider the abstract graph defined by degrees of freedom ([DOFs](../overview.html#elemdof)) and constraints. A vertex is all the
DOFs in a single kinematic [tree](../overview.html#elemtree); an edge is a constraint (a contact, equality or tendon limit) between
...
memory-contiguous. This amounts to a block-diagonalization of the constraint Jacobian \(J\), as illustrated in the
figure. On the left is the monolithic Jacobian of size \(\nc \times \nv\), where we use the [corresponding](#framework) size names from MuJoCo’s data structures `mjData.nefc` and `mjModel.nv`. On the right is the
...
While islanding is not free (see implementation in [engine\_island.c](https://github.com/google-deepmind/mujoco/blob/main/src/engine/engine_island.c)), it is worth the effort:
...
Here we explain how the quantities \(R, \ar\) are computed from model parameters. For the chosen
parameterization to make sense, we first need to understand how these quantities affect the dynamics. We focus on the
unconstrained minimizer of [(9)](#equation-eq-dual), namely

\[f^+ = (A+R)^{-1} (\ar - \au)
\]

If it happens that \(f^+ \in \Omega\), then \(f^+ = f\) is the actual constraint force generated by our model.
We focus on this case because it is common, in the sense that the subset of the constraints in \(\Omega\) that are
active at any given time is usually small, and furthermore it is the only case that we can actually analyze.
Substituting \(f^+\) in the constraint dynamics [(10)](#equation-eq-identity) and rearranging terms yields
...
in the limit \(R \to 0\) we have a hard constraint and \(\ac = \ar\), while in the limit \(R \to \infty\) we
have have an infinitely soft constraint (i.e., no constraint) and \(\ac = \au\). It is then natural to introduce a
model parameter which directly controls the interpolation. We call this parameter *impedance* and denote it \(d\).
It is a vector with dimensionality \(\nc\) satisfying \(0<d<1\) element-wise. Once it is specified, we compute
the diagonal elements of the regularizer as

(11)[#](#equation-eq-impedance-r "Permalink to this equation")\[R\_{ii} = \frac{1-d\_i}{d\_i} \hat{A}\_{ii}\]
...
near-zero despite finite directional inertia — the regularizer \(R\) becomes near-zero, making constraints
infinitely hard and causing divergence. The [diagexact](../XMLreference.html#option-flag-diagexact) flag replaces the approximation with
the exact diagonal \(A\_{ii} = \|Y\_i\|^2\), where \(Y = J M^{-1/2}\) is the whitened Jacobian, computed at the
current configuration. This eliminates all three sources of error at a modest runtime cost: computing \(Y\) requires
a back-substitution with the Cholesky factor of the mass matrix for each active constraint row; if
[dual solvers](#soalgorithms) are used (PGS or NoSlip), the cost is negligible since \(Y\) is computed anyway.
...
(12)[#](#equation-eq-aref "Permalink to this equation")\[\ari = -b\_i (J v)\_i - k\_i r\_i\]

Recall that \(r\) is the position residual, while \(J v\) is the joint velocity projected in constraint space;
the indexing notation refers to one component of the projected velocity vector. For friction loss and friction
dimensions of elliptic cones, \(r \equiv 0\) and therefore \(k=0\), so the reference acceleration reduces to
pure damping: \(\ari = -b\_i (J v)\_i\). More detail is given in the [Friction](../modeling.html#csolverfriction) section of the
Modeling chapter.

To summarize, the constraint behavior is determined by three per-constraint quantities: impedance \(0<d<1\), damping
\(b > 0\), and stiffness \(k \geq 0\). These are computed from the solimp and solref attributes as
described in the [solver parameters](../modeling.html#sorefscaling) section of the Modeling chapter, which also offers additional
automation (e.g., achieving critical damping, or varying \(d\) with distance to model a soft contact layer). The
quantities \(R, \ar\) are then computed from [(11)](#equation-eq-impedance-r) and [(12)](#equation-eq-aref), and the selected optimization
algorithm is applied to solve problem [(9)](#equation-eq-dual).

The closed-loop constraint dynamics resulting from the combination of \(R\) and \(\ar\) are analyzed in
detail in the [Solver parameters](../modeling.html#csolver) section of the Modeling chapter. In brief, each scalar constraint
behaves approximately as a damped second-order system whose time constant and damping ratio are set by the solref
attribute, and whose strength is controlled by the impedance \(d\) set via solimp. When critically damped
(\(\text{dampratio} = 1\)), the steady-state penetration under a constant external load is independent of the
effective mass in constraint space – a consequence of the impedance-scaled parameterization.

### Friction cones[#](#socones "Link to this heading")

As explained above, MuJoCo allows both elliptic friction cones and pyramidal approximations to them; the selected solver
determines which type of friction cone is used. The pyramidal approximation has \(2 (n-1)\) edges where \(n\)
is the dimensionality of the contact space as specified by condim. We could add more edges yielding better
...
our optimization problem [(7)](#equation-eq-primal) would converge to the solution for the elliptic cone. This is true in the limit
...
fixing the acceleration variables \(x\) in problem [(7)](#equation-eq-primal) and optimize away the deformation variables
...
[![../_images/softcontact.png](../_images/softcontact.png)](../_images/softcontact.png)
[![../_images/softcontact_dark.png](../_images/softcontact_dark.png)](../_images/softcontact_dark.png)
...
## Collision detection[#](#collision-detection "Link to this heading")
...
### Selection[#](#selection "Link to this heading")

If a model has \(n\) geoms, there are \(n (n-1)/2\) geoms pairs that can potentially collide. Checking all these
...
:   First we generate a list of candidate geom pairs by merging from two sources: pairs of bodies that might contain
    colliding geoms and the explicit list of geom pairs defined with the [pair](../XMLreference.html#contact-pair) element in MJCF.
    ...
    Finally, the user can explicitly exclude certain body pairs using the [exclude](../XMLreference.html#contact-exclude) element in
    MJCF. At the end of this step we have a list of geoms pairs that is typically much smaller than \(n (n-1)/2\),
...
### Checking[#](#checking "Link to this heading")

Detailed collision checking, also known as *near-phase* or [narrow-phase](https://en.wikipedia.org/wiki/Collision_detection#Narrow_phase) collision detection, is performed by functions
that depend on the geom types in the pair. The table of narrow-phase collision functions can be inspected at the top of
[engine\_collision\_driver.c](https://github.com/google-deepmind/mujoco/blob/main/src/engine/engine_collision_driver.c)
and exposed to users who wish to install their own colliders as [mjCOLLISIONFUNC](../APIreference/APIglobals.html#mjcollisionfunc). MuJoCo supports several
primitive geometric shapes: plane, sphere, capsule, cylinder, ellipsoid, and box. It also supports triangulated meshes and
height-fields.

With the notable exception of [SDF plugins](../programming/extension.html#exsdf) (see documentation therein), collision detection is limited to
...
hulls (visualized with the ‘H’ key in [simulate](../programming/samples.html#sasimulate)), computed by the [qhull](http://www.qhull.org/)
library.

#### Convex collisions[#](#convex-collisions "Link to this heading")
...
:   The native CCD pipeline (“nativeccd”) is implemented natively in MuJoCo, based on the Gilbert-Johnson-Keerthi and
    Expanding Polytope algorithms ([GJK](https://en.wikipedia.org/wiki/Gilbert%E2%80%93Johnson%E2%80%93Keerthi_distance_algorithm) / [EPA](http://scroll.stanford.edu/courses/cs468-01-fall/Papers/van-den-bergen.pdf)). The native pipeline is both faster and more robust than the MPR-based
    pipeline.

libccd pipeline (legacy)
:   This legacy pipeline is based on the [libccd](https://github.com/danfis/libccd) library, and uses Minkowski Portal Refinement ([MPR](https://en.wikipedia.org/wiki/Minkowski_Portal_Refinement)). It is activated by
    disabling the [nativeccd](../XMLreference.html#option-flag-nativeccd) flag.

Both pipelines are controlled by a tolerance (in units of distance) and maximum iteration parameters exposed as
`mjOption.ccd_tolerance` ([ccd\_tolerance](../XMLreference.html#option-ccd-tolerance)) and `mjOption.ccd_iterations`
([ccd\_iterations](../XMLreference.html#option-ccd-iterations)), respectively.

#### Multiple contacts[#](#multiple-contacts "Link to this heading")
...
[multiccd](../XMLreference.html#option-flag-multiccd) flag, but is implemented in different ways with different trade-offs:
...
:   Multiple contact points are found by rotating the two geoms by ±1e-3 radians around the tangential axes and
    ...
    used when the [nativeccd](../XMLreference.html#option-flag-nativeccd) flag is disabled, and for geoms collisions involving cylinders
    and capsules or with [positive contact margins](../XMLreference.html#body-geom-margin).

single-shot pipeline
:   The single-shot pipeline is used in conjunction with the native CCD pipeline, i.e., when the
    [nativeccd](../XMLreference.html#option-flag-nativeccd) flag is enabled. As this pipeline is one-shot and most of the geom analysis
    is done at compilation time, there is very little performance overhead. Supported geoms are boxes and meshes without
    [positive contact margins](../XMLreference.html#body-geom-margin).

[![../_images/ccd_light.gif](../_images/ccd_light.gif)](../_images/ccd_light.gif)
[![../_images/ccd_dark.gif](../_images/ccd_dark.gif)](../_images/ccd_dark.gif)

#### Geom distance[#](#geom-distance "Link to this heading")

The narrow-phase collision functions described [above](#cochecking) drive the [mj\_geomDistance](../APIreference/APIfunctions.html#mj-geomdistance) function and
associated [collision sensors](../XMLreference.html#collision-sensors). Due to the limitations of MPR, the legacy pipeline will return incorrect values
...
#### Convex decomposition[#](#convex-decomposition "Link to this heading")
...
height fields) are [Signed Distance Functions](../programming/extension.html#exsdf) (see documentation therein), which in certain cases (e.g.,
[analytic SDFs](https://github.com/google-deepmind/mujoco/blob/main/plugin/sdf/README.md#gear)) can be efficient, but
have other requirements and limitations.

Open source mesh-decomposition tools like the [CoACD library](https://github.com/SarahWeiii/CoACD) can be used
...
#### Pair-wise colliders[#](#pair-wise-colliders "Link to this heading")

The table below provides information about the colliders used for different geom pairs. These values can be computed
dynamically by the [mj\_maxContact](../APIreference/APIfunctions.html#mj-maxcontact) function. Use the toggles to see the max number of contacts returned with the
parameters [nativeccd](../XMLreference.html#option-flag-nativeccd), [multiccd](../XMLreference.html#option-flag-multiccd), and
[margin](../XMLreference.html#body-geom-margin).
...
| HField | HFieldCCD  [mjMAXCONPAIR](../APIreference/APIglobals.html#glnumericengine) | HFieldCCD  [mjMAXCONPAIR](../APIreference/APIglobals.html#glnumericengine) | HFieldCCD  [mjMAXCONPAIR](../APIreference/APIglobals.html#glnumericengine) | HFieldCCD  [mjMAXCONPAIR](../APIreference/APIglobals.html#glnumericengine) | HFieldCCD  [mjMAXCONPAIR](../APIreference/APIglobals.html#glnumericengine) | HFieldCCD  [mjMAXCONPAIR](../APIreference/APIglobals.html#glnumericengine) | HFieldSDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |
| Sphere | primitive  **1** | primitive  **1** | CCD  **1** | primitive  **1** | primitive  **1** | CCD  **1** | SDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |
| Capsule |  | primitive  **2** | CCD  **1** | CCD  **1**  **5**  **5** | primitive  **2** | CCD  **1**  **5**  **5** | SDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |
| Ellipsoid |  |  | CCD  **1** | CCD  **1** | CCD  **1** | CCD  **1** | SDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |
| Cylinder |  |  |  | CCD  **1**  **5**  **5** | CCD  **1**  **5**  **5** | CCD  **1**  **5**  **5** | SDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |
| Box |  |  |  |  | primitive  **8** | CCD  **1**  **4**  **5** | SDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |
| Mesh |  |  |  |  |  | CCD  **1**  **4**  **5** | MeshSDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |
| SDF |  |  |  |  |  |  | SDF  [sdf\_initpoints](../XMLreference.html#option-sdf-initpoints) |

## Sleeping islands[#](#sleeping-islands "Link to this heading")
...
number of passive objects. The smallest unit which can be asleep or awake is a [kinematic tree](../overview.html#elemtree), however
trees are always put to sleep together with other trees to which they are connected by constraints, hence the term
“sleeping [islands](#soisland)”.

The video on the right demonstrates several aspects of sleeping. First we show the [dominos](https://github.com/google-deepmind/mujoco/blob/main/model/sleep/dominos.xml) model, which simulates a traditional
...
shows a variant of the [100 humanoids](https://github.com/google-deepmind/mujoco/blob/main/model/sleep/100_humanoids.xml) model, where all humanoids are
...
[position stage](#pistages) of the pipeline it is effectively instantaneous, and the woken island will behave
exactly as if it was awake all along.

Sleeping is off by default and enabled using the [sleep](../XMLreference.html#option-flag-sleep) flag. A detailed description of the
sleeping mechanism is provided in the [Simulation chapter](../programming/simulation.html#sisleep) but here we provide a brief overview.

Sleeping can occur in one of two ways:

* **Automatic:** A tree whose maximum velocity in absolute value is less than the
  [tolerance](../XMLreference.html#option-sleep-tolerance) for [mjMINAWAKE](../APIreference/APIglobals.html#glnumericengine) time steps is marked as “ready to sleep”.
  If all trees in an island are ready to sleep, they are put to sleep during state advancement.
* **Initialized asleep:** By setting the [body/sleep](../XMLreference.html#body-sleep) attribute of a tree root to “init”, it is
  marked as “initialized-asleep” and put to sleep during [mjData](../APIreference/APItypes.html#mjdata) initialization.

## Simulation pipeline[#](#simulation-pipeline "Link to this heading")
...
### Forward dynamics[#](#forward-dynamics "Link to this heading")

The source file [engine\_forward.c](https://github.com/google-deepmind/mujoco/blob/main/src/engine/engine_forward.c)
contains the high-level forward dynamics pipeline.

#### Top level[#](#top-level "Link to this heading")

* The top-level function [mj\_step](../APIreference/APIfunctions.html#mj-step) invokes the entire sequence of stages below **1-26**.
* [mj\_forward](../APIreference/APIfunctions.html#mj-forward) invokes only stages **2-23**, computing the continuous-time forward dynamics, ending with the
  acceleration `mjData.qacc`.
* [mj\_step1](../APIreference/APIfunctions.html#mj-step1) invokes stages **1-19** and [mj\_step2](../APIreference/APIfunctions.html#mj-step2) invokes stages **20-26**, breaking [mj\_step](../APIreference/APIfunctions.html#mj-step) into two
  distinct phases. This allows the user to write controllers that depend on quantities derived from the positions and
  velocities (but not forces, since those have not yet been computed). Note that the [mj\_step1](../APIreference/APIfunctions.html#mj-step1) → [mj\_step2](../APIreference/APIfunctions.html#mj-step2)
  pipeline does not support the Runge Kutta integrator.
* [mj\_fwdPosition](../APIreference/APIfunctions.html#mj-fwdposition) invokes stages **2-11**, the position-dependent part of the pipeline.

Schematic breakdown of the forward dynamic pipeline:

| top-level functions | [mj\_step](../APIreference/APIfunctions.html#mj-step) | | | | | | | | |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| [mj\_step1](../APIreference/APIfunctions.html#mj-step1) | | | | | | [mj\_step2](../APIreference/APIfunctions.html#mj-step2) | | |
| [mj\_forward](../APIreference/APIfunctions.html#mj-forward) | | | | | | |  |  |
| component / description |  | [fwdPosition](../APIreference/APIfunctions.html#mj-fwdposition) | |  | [fwdVelocity](../APIreference/APIfunctions.html#mj-fwdvelocity) |  |  |  |  |
|  | [fwdKinematics](../APIreference/APIfunctions.html#mj-fwdkinematics) | inertia, collision |  |  |  | acceleration |  | advance |
| stage | 1 | 2-5 | 6-11 | 12 | 13-18 | 19 | 20-23 | 24,25 | 26 |

#### Stages[#](#stages "Link to this heading")

Below we describe the pipeline stages and API functions corresponding to each stage. All functions write their outputs
into attributes of [mjData](../APIreference/APItypes.html#mjdata). It is informative to compare the list below with the [mjData](../APIreference/APItypes.html#mjdata) struct definition,
...
1. Check the positions and velocities for invalid or unacceptably large real values indicating divergence. If divergence
   ...
   [mj\_checkPos](../APIreference/APIfunctions.html#mj-checkpos), [mj\_checkVel](../APIreference/APIfunctions.html#mj-checkvel)
...
2. Compute the forward kinematics. This yields the global positions and orientations of all bodies, geoms, sites,
   cameras and lights. It also normalizes all quaternions: [mj\_kinematics](../APIreference/APIfunctions.html#mj-kinematics), [mj\_camlight](../APIreference/APIfunctions.html#mj-camlight)
3. Compute the body inertias and joint axes, in global frames centered at the centers of mass of the corresponding
   kinematic subtrees: [mj\_comPos](../APIreference/APIfunctions.html#mj-compos)
4. Compute quantities related to [flex](../XMLreference.html#deformable-flex) objects: [mj\_flex](../APIreference/APIfunctions.html#mj-flex)
5. Compute the tendon lengths and moment arms. This includes the computation of minimal-length paths for spatial
   tendons: [mj\_tendon](../APIreference/APIfunctions.html#mj-tendon)
6. Compute the composite rigid body inertias and joint-space inertia matrix: [mj\_makeM](../APIreference/APIfunctions.html#mj-makem)
7. Compute the sparse factorization of the joint-space inertia matrix: [mj\_factorM](../APIreference/APIfunctions.html#mj-factorm)
8. Construct the list of active contacts. This includes both broad-phase and near-phase collision detection:
   [mj\_collision](../APIreference/APIfunctions.html#mj-collision)
9. Construct the constraint Jacobian, compute the constraint residuals, construct islands: [mj\_makeConstraint](../APIreference/APIfunctions.html#mj-makeconstraint),
   `mj_island` (not yet exposed in the API)
10. Compute the actuator lengths and moment arms: [mj\_transmission](../APIreference/APIfunctions.html#mj-transmission)
11. Compute the matrices and vectors needed by the constraint solvers: [mj\_projectConstraint](../APIreference/APIfunctions.html#mj-projectconstraint)
12. Compute sensor data that only depends on position, and the potential energy if enabled: [mj\_sensorPos](../APIreference/APIfunctions.html#mj-sensorpos),
    [mj\_energyPos](../APIreference/APIfunctions.html#mj-energypos)

##### Velocity[#](#velocity "Link to this heading")
...
13. Compute the tendon, flex edge and actuator velocities: [mj\_fwdVelocity](../APIreference/APIfunctions.html#mj-fwdvelocity)
14. Compute the body velocities and rates of change of the joint axes, again in the global coordinate frames centered at
    the subtree centers of mass: [mj\_comVel](../APIreference/APIfunctions.html#mj-comvel)
15. Compute passive forces – spring-dampers in joints and tendons, and fluid forces: [mj\_passive](../APIreference/APIfunctions.html#mj-passive)
16. Compute sensor data that depends on velocity, and the kinetic energy if enabled
    (if required by sensors, call [mj\_subtreeVel](../APIreference/APIfunctions.html#mj-subtreevel)): [mj\_sensorVel](../APIreference/APIfunctions.html#mj-sensorvel)
17. Compute the reference constraint acceleration: [mj\_referenceConstraint](../APIreference/APIfunctions.html#mj-referenceconstraint)
18. Compute the vector of Coriolis, centrifugal and gravitational forces: [mj\_rne](../APIreference/APIfunctions.html#mj-rne)

##### Control callback[#](#control-callback "Link to this heading")

19. Invoke the user-defined control callback if defined: [mjcb\_control](../APIreference/APIglobals.html#mjcb-control)

##### Force/acceleration[#](#force-acceleration "Link to this heading")

The stages below compute quantities that depend on [user inputs](../programming/simulation.html#siinput). Due to the sequential nature
of the pipeline, the actual dependence is on the entire [integration state](../programming/simulation.html#siintegrationstate).

20. Compute the actuator forces and activation dynamics if defined: [mj\_fwdActuation](../APIreference/APIfunctions.html#mj-fwdactuation)
21. Compute the joint acceleration resulting from all forces except for the (still unknown) constraint forces:
    [mj\_fwdAcceleration](../APIreference/APIfunctions.html#mj-fwdacceleration)
22. Compute the constraint forces with the selected solver, and update the joint acceleration so as to account for the
    ...
    [mj\_fwdConstraint](../APIreference/APIfunctions.html#mj-fwdconstraint)
23. Compute sensor data that depends on force and acceleration if enabled
    (if required by sensors, call [mj\_rnePostConstraint](../APIreference/APIfunctions.html#mj-rnepostconstraint)): [mj\_sensorAcc](../APIreference/APIfunctions.html#mj-sensoracc)
24. Check the acceleration for invalid or unacceptably large real values. If divergence is detected, the state is
    automatically reset and the corresponding warning is raised: [mj\_checkAcc](../APIreference/APIfunctions.html#mj-checkacc)
25. Compare the results of forward and inverse dynamics, so as to diagnose poor solver convergence in the forward
    dynamics. This is an optional step, and is performed only when enabled: [mj\_compareFwdInv](../APIreference/APIfunctions.html#mj-comparefwdinv)
26. Advance the simulation state by one time step, using the selected integrator. Note that the Runge-Kutta integrator
    ...
    one of [mj\_Euler](../APIreference/APIfunctions.html#mj-euler), [mj\_RungeKutta](../APIreference/APIfunctions.html#mj-rungekutta), [mj\_implicit](../APIreference/APIfunctions.html#mj-implicit)

### Consistency in `mjData`[#](#consistency-in-mjdata "Link to this heading")
...
* After setting [the state](#gestate), state-derived quantities do not automatically correspond to the new state.
  ...
  [mj\_kinematics](../APIreference/APIfunctions.html#mj-kinematics).
* After an [mj\_step](../APIreference/APIfunctions.html#mj-step), which terminates immediately after updating the state, quantities in `mjData` correspond
  ...
  [Jacobians](../APIreference/APIfunctions.html#mj-jac), will be with respect to the *previous positions*.

### Reproducibility[#](#reproducibility "Link to this heading")

MuJoCo’s simulation pipeline is entirely deterministic and reproducible – if a [state](#gestate) in a trajectory is
saved and reloaded and [mj\_step](../APIreference/APIfunctions.html#mj-step) called again, the resulting next state will be identical. However, there are some
important caveats:

* Save all the required [integration state](../programming/simulation.html#siintegrationstate) components. In particular [warmstart
  accelerations](../programming/simulation.html#siwarmstart) have only a very small effect on the next state, but should be saved if bit-wise equality
  is required.
* Any numerical difference between states, no matter how small, will become significant upon integration, especially for
  systems with contact. Contact events have high [Lyapunov exponents](https://en.wikipedia.org/wiki/Lyapunov_exponent); this is a property of any rigid-body simulator (and indeed of
  [real-world physics](https://en.wikipedia.org/wiki/Roulette)) and is not MuJoCo-specific.
...
### Inverse dynamics[#](#inverse-dynamics "Link to this heading")

The top-level function [mj\_inverse](../APIreference/APIfunctions.html#mj-inverse) invokes the following sequence of computations. The notes above regarding
[consistency](#piconsistency) and [reproducibility](#pireproducibility) apply here as well.
...
12. Compute sensor data that depends on velocity, and the kinetic energy if enabled.
    If required by sensors, call [mj\_subtreeVel](../APIreference/APIfunctions.html#mj-subtreevel).
13. Compute all passive forces.
14. Compute the reference constraint acceleration.
15. If the [invdiscrete](../XMLreference.html#option-flag-invdiscrete) flag is set and the [integrator](../XMLreference.html#option-integrator) is not
...
18. Compute sensor data that depends on force and acceleration if enabled.
    If required by sensors, call [mj\_rnePostConstraint](../APIreference/APIfunctions.html#mj-rnepostconstraint).
19. Compute the vector `mjData.qfrc_inverse` by combining all results. This is the main output of inverse dynamics. It
...
[implicit integrators](#geintegration).

Note that the default value of the [solver impedance](../modeling.html#csolverimpedance) is such that contacts are *not*
differentiable by default, and needs to be [set to 0](../modeling.html#solimp0) in order for contact-force onset to be smooth.

Two functions are currently available which use efficient finite-differencing in order to compute dynamics Jacobians:

[mjd\_transitionFD](../APIreference/APIfunctions.html#mjd-transitionfd):
:   Computes state-transition and control-transition Jacobians for the discrete-time forward dynamics ([mj\_step](../APIreference/APIfunctions.html#mj-step)).
    See [documentation](../APIreference/APIfunctions.html#mjd-transitionfd).

[mjd\_inverseFD](../APIreference/APIfunctions.html#mjd-inversefd):
:   Computes Jacobians for the continuous or discrete-time inverse dynamics ([mj\_inverse](../APIreference/APIfunctions.html#mj-inverse)).
    See [documentation](../APIreference/APIfunctions.html#mjd-inversefd).
...
* Computation
  ...
  + [Soft contact model](#soft-contact-model)
    - [Physical realism and soft contacts](#physical-realism-and-soft-contacts)
    - [Computational efficiency](#computational-efficiency)
    - [Continuous time](#continuous-time)
    - [Inverse dynamics and optimization](#inverse-dynamics-and-optimization)
  + [General framework](#general-framework)
    - [Actuation model](#actuation-model)
      * [Transmission](#transmission)
      * [Stateful actuators](#stateful-actuators)
      * [Force generation](#force-generation)
    - [Passive forces](#passive-forces)
      * [Polynomial forces](#polynomial-forces)
  ...
  + [Constraint model](#constraint-model)
    ...
    - [Limit](#limit)
    - [Contact](#contact)
      * [margin and gap](#margin-and-gap)
      * [condim](#condim)
      * [Friction cones](#friction-cones)
  + [Constraint solver](#constraint-solver)
    - [Primal problem](#primal-problem)
    - [Reduced primal problem](#reduced-primal-problem)
    - [Dual problem](#dual-problem)
    - [Algorithms](#algorithms)
    - [Constraint islands](#constraint-islands)
    - [Parameters](#parameters)
    - [Friction cones](#socones)
  + [Collision detection](#collision-detection)
    - [Selection](#selection)
    - [Checking](#checking)
      * [Convex collisions](#convex-collisions)
      * [Multiple contacts](#multiple-contacts)
      * [Geom distance](#geom-distance)
      * [Convex decomposition](#convex-decomposition)
      * [Pair-wise colliders](#pair-wise-colliders)
  + [Sleeping islands](#sleeping-islands)
  + [Simulation pipeline](#simulation-pipeline)
    ...
    - [Consistency in `mjData`](#consistency-in-mjdata)
