...
这篇 ICLR 2026 论文提出 [[disentangled-robot-learning-via-separate-forward-and-inverse-dynamics-pretraining|DeFI]]：把机器人策略学习拆成两个先分开预训练、再耦合微调的动力学模块。[[InverseDynamicsModels|逆动力学模型]] 不再只是一个轻量动作解码器，而是和正向动力学一样被当成需要规模预训练的核心能力。
...
阶段 II 中，GFDM 冻结的，只做单一步骤 denoising 产生 16-帧未来表征；这些未来特征经 MLP 对齐到 GIDM 输入流形，GIDM 推断潜在动作序列，再由扩散基于动作 adapter 转成可执行的机器人命令。论文的核心判断是：大规模无动作标注的人类/视频数据不只适合学正向视频预测，也可以通过 self-supervised 逆动力学预训练学到动作表示。
...
- Entangled VLA 训练把 2D 未来预测和 3D 动作预测绑在一起，容易产生目标 misalignment，也限制模型使用无动作标注的 web/人类视频；DeFI 改成先分开学 GFDM/GIDM，再在下游机器人数据上耦合。
...
- [[disentangled-robot-learning-via-separate-forward-and-inverse-dynamics-pretraining|DeFI]] - 本来源的核心模型/实体。
...
- GIDM 预训练不使用动作/proprio 标签，但最终策略仍依赖动作 adapter 在下游机器人动作数据上语义落地。它学到的是 transferable 潜在动作表示，不等于完全无动作标注的策略学习。
