...
type: source
tags: [robotics, simulation, embodied-ai, data-generation, systems, source-backed]
sources: []
modified: 2026-09-25
...
Haoran Lu 等提出 [[magicsim-a-unified-infrastructure-for-executable-embodied-interaction|MagicSim]]：一套构建在 Isaac Sim / PhysX / RTX 之上的可执行具身交互基础设施。它的核心不是新的物理引擎、规划算法或策略模型，而是把异构世界构建、确定性批处理运行时、多机器人控制、规划器在线调用、任务 MDP、自动轨迹采集、标注与智能体接口组织在同一个回合生命周期中。论文用“一套运行时、一个 MDP、三种驱动器”概括设计：同一任务可以被 RL 策略直接执行、被 AutoCollect 的命令—技能—规划器层执行，或通过计划中的推理 / 重放驱动器执行。
